...
%% @doc NkSERVER OpenTracing
-module(nkserver_ot_app).
...
start(Type) ->
    case application:ensure_started(?APP, Type) of
        {ok, _Started} ->
            ok;
        Error ->
            Error
...
start(_Type, _Args) ->
    ...
    case nklib_config:load_env(?APP, Syntax) of
        {ok, _} ->
            {ok, Pid} = nkserver_ot_sup:start_link(),
            {ok, Vsn} = application:get_key(nkserver_ot, vsn),
            lager:info("NkSERVER OpenTracing v~s has started.", [Vsn]),
            {ok, Pid};
        {error, Error} ->
